...
export default function RoutingNumberCard({ data }: { data: RoutingData }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <div className="bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl p-5 md:p-8 shadow-sm flex flex-col items-center justify-between gap-6 print:border-none print:shadow-none print:p-0">
        <div className="text-center w-full">
          <div className="flex flex-col sm:flex-row sm:items-center justify-center md:justify-between gap-4 mb-4">
            <div className="text-[10px] sm:text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] md:text-left">
          ...
          <div className="mt-6 flex flex-wrap gap-2 justify-center md:justify-start">
            {(type === 'ACH' || type === 'BOTH') && (
              <span className="bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800 text-[10px] sm:text-xs font-bold px-3 py-1.5 rounded-full uppercase flex items-center gap-1.5 whitespace-nowrap">
                <span className="w-2 h-2 rounded-full bg-blue-500 dark:bg-blue-400"></span> ACH Ready
              </span>
            )}
            {(type === 'WIRE' || type === 'BOTH') && (
              <span className="bg-orange-100 dark:bg-orange-900/40 text-orange-800 dark:text-orange-300 border border-orange-200 dark:border-orange-800 text-[10px] sm:text-xs font-bold px-3 py-1.5 rounded-full uppercase flex items-center gap-1.5 whitespace-nowrap">
                <span className="w-2 h-2 rounded-full bg-orange-500 dark:bg-orange-400"></span> Wire Transfer
              </span>
            )}
            <span className="bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-600 text-[10px] sm:text-xs font-bold px-3 py-1.5 rounded-full uppercase flex items-center gap-1.5 whitespace-nowrap">
